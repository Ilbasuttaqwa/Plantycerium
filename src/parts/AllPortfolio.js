...
export default function AllPortfolio({ data }) {
  const wandae = data.filter((item) => item.type === "P.Wandae");
  const grande = data.filter((item) => item.type === "P.Grande");
  const willinckii = data.filter((item) => item.type === "Willinckii");
